...
export default {
	...
	methods: {
		getDecks: function() {
			axios.get('http://188.186.86.13/api/decks')
			.then(response => {
		...
		changeDeckStatus: function(deck){
			axios.get('http://188.186.86.13/api/decks/'+deck.id+'/changeStatus')
				.then(response => {
                    this.getDecks();
                });
		},
		uploadCard: function() {
			if(this.validateUploadForm()){
				...
				axios.post('http://188.186.86.13/api/cards',formData,{
		...
		deleteCard: function(user){
            axios.delete('http://188.186.86.13/api/decks/'+user.id)
                .then(response => {
                });
		},
		createDeck: function(){
			if(! this.newDeckTitle == ''){
				...
				axios.post('http://188.186.86.13/api/decks',formData,{
					'Content-Type': 'multipart/form-data'	
				})
				.then(response => {
					this.newDeckTitle = '';
