...
export default {
	...
	methods: {
		getStats: function() {
			axios.get('http://128.199.51.26/api/stats/totalPlayers')
			.then(response => {
				this.totalNrPlayers = response.data;
				console.log(response.data);
            });

            axios.get('http://128.199.51.26/api/stats/totalGames')
			.then(response => {
				this.totalNrPlayedGames = response.data;
				console.log(response.data);
            });

			axios.get('http://128.199.51.26/api/stats/topPlayersMorePoints')
			.then(response => {
				this.topPlayersPoints = response.data.data;
				console.log(response.data);
            });

            axios.get('http://128.199.51.26/api/stats/topPlayersMoreGames')
			.then(response => {
				this.topPlayersGames = response.data.data;
				console.log(response.data);
            });

			axios.get('http://128.199.51.26/api/stats/topPlayersBestAvg')
			.then(response => {
